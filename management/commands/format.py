...
class FormattingConfig:
    ...
    IMPORT_SECTION: ClassVar[Pattern[str]] = re.compile(r'((?:^(?:from\s+\S+\s+)?import\s+.+\n)+)(\n*)', re.MULTILINE)
    EXCESSIVE_BLANKS: ClassVar[Pattern[str]] = re.compile(r'\n{3,}')
    CLASS_DEFINITION: ClassVar[Pattern[str]] = re.compile(r'\n{3,}(class\s+\w+)')
    FUNCTION_DEFINITION: ClassVar[Pattern[str]] = re.compile(r'\n{3,}(def\s+\w+)')
