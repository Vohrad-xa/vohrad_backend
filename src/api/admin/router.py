...
async def list_users(params=Depends(get_admin_params), scope: Optional[str] = Query(None)):
    ...
    return await admin_service.paginated_call(
        context, user_service.get_multi, pagination, UserResponse, tenant_id=context.tenant_id, scope=scope
    )
...
async def delete_tenant(tenant_id: UUID, params=Depends(get_admin_params)) -> DeletedResponse:
    """Delete tenant."""
    _, context = params
