...
class ErrorRegistry:
    ...
    ENTITY_NOT_FOUND: Final[ErrorDefinition] = ErrorDefinition(
        code="ENTITY_NOT_FOUND", message="Resource not found", status_code=status.HTTP_404_NOT_FOUND, category="domain"
    )
    ...
    AUTHORIZATION_FAILED: Final[ErrorDefinition] = ErrorDefinition(
        code="AUTHORIZATION_FAILED", message="Access denied", status_code=status.HTTP_403_FORBIDDEN, category="auth"
    )
